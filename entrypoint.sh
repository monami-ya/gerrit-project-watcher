...
GERRIT_MASTER_URL="${GERRIT_MASTER_URL_SCHEME}://${GERRIT_MASTER_HOST}:${GERRIT_MASTER_PORT}${GERRIT_MASTER_API_PATH}"
...
while read repo; do
    ...
    echo "${repo} $?"
done < <(curl -sL --user "$GERRIT_MASTER_CRED" ${GERRIT_MASTER_URL} | \
    tail -c +6 | \
